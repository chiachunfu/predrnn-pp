...
class STLSTMCell():
    ...
    def __call__(self, x, h, c, m):
        ...
        with tf.variable_scope(self.layer_name):
            ...
            #i_c, g_c, f_c = tf.split(c_cc, 3, 3)
            #i_c, g_c, f_c = tf.split(c_cc, 3, 3)
            i_m, f_m, m_m, g_m = tf.split(m_cc, 4, 3)

            if x is None:
                i = tf.sigmoid(i_h + self._input_bias)
                f = tf.sigmoid(f_h + self._forget_bias)
                g = tf.tanh(g_h + self._input_mod_bias)
            else:
                x_cc = tf.layers.conv2d(
                    x, self.num_hidden*7,
                    self.filter_size, 1, padding='same',
                ...
                i = tf.sigmoid(i_x + i_h + self._input_bias)
                f = tf.sigmoid(f_x + f_h + self._forget_bias)
                g = tf.tanh(g_x + g_h + self._input_mod_bias)
            ...
            if x is None:
                ii = tf.sigmoid(i_m + self._input_bias)
                ff = tf.sigmoid(f_m + self._forget_bias)
                gg = tf.tanh(g_m + self._input_mod_bias)
            else:
                ii = tf.sigmoid(i_x_ + i_m + self._input_bias)
                ff = tf.sigmoid(f_x_ + f_m + self._forget_bias)
                gg = tf.tanh(g_m + g_x_ + self._input_mod_bias)
            ...
            if x is None:
                o = tf.tanh(o_h + c2m + o_m + self._output_bias)
            else:
                o = tf.tanh(o_x + o_h + c2m + o_m + self._output_bias)
